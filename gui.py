# main.py (четыре колонки: управление/параметры, лог, заданные выходы, ожидаемые выходы, бинарные входы)
import time
import threading
import queue
import os
...
from ModesHandler import ModesHandler
...
class RetomApp:
    def __init__(self, root):
        self.root = root
        self.root.title("RETOM-71 Controller v1.5")
        self.root.geometry("2400x1150")
        ...
        self.setpoint_outputs_state = [False] * 16  # Заданные выходы
        self.expected_outputs_state = [False] * 16  # Ожидаемые выходы
        ...
        # === ВАЖНО: Обновляем GUI после инициализации состояний ===
        self._update_setpoint_outputs_gui()   # Обновим заданные выходы (должны загореться)
        self._update_expected_outputs_gui()   # Обновим ожидаемые выходы (останутся выключенными)
    ...
    def _setup_ui(self):
        ...
        # Панель управления (первый ряд кнопок)
        ...
        for label, cmd in buttons_config:
            ...
            btn.pack(side=tk.LEFT, padx=5)
                ...
                # ВТОРОЙ РЯД КНОПОК: Load Mode, Load Modes, Run Autotest
        # В _setup_ui, замените блок с autotest_frame на этот:

        # ВТОРОЙ РЯД КНОПОК: Load Mode, Load Modes, Run Autotest
        autotest_frame = ttk.LabelFrame(column1, text="Автоматизация", padding="5")
        autotest_frame.pack(fill=tk.X, pady=(0, 10))

        # Кнопка Load Mode (один режим)
        btn_load_mode = ttk.Button(autotest_frame, text="Load Mode", 
                                command=self._load_mode)
        btn_load_mode.pack(side=tk.LEFT, padx=5)

        # Кнопка Load Modes (несколько режимов)
        btn_load_modes = ttk.Button(autotest_frame, text="Load Modes", 
                                    command=self._load_modes)
        btn_load_modes.pack(side=tk.LEFT, padx=5)

        # Кнопка Run Autotest
        btn_run_autotest = ttk.Button(autotest_frame, text="Run Autotest", 
                                    command=self._run_autotest)
        btn_run_autotest.pack(side=tk.LEFT, padx=5)

        # Разделитель
        ttk.Separator(autotest_frame, orient=tk.VERTICAL).pack(side=tk.LEFT, padx=10, fill=tk.Y)

        # Панель управления режимами
        mode_control_frame = ttk.Frame(autotest_frame)
        mode_control_frame.pack(side=tk.LEFT, padx=(5, 0))

        # Метка "Режим:"
        ttk.Label(mode_control_frame, text="Режим:", font=("Arial", 9, "bold")).pack(side=tk.LEFT, padx=(0, 5))

        # Кнопка "←"
        self.btn_prev_mode = ttk.Button(mode_control_frame, text="◀", width=3, 
                                        command=self._prev_mode, state=tk.DISABLED)
        self.btn_prev_mode.pack(side=tk.LEFT, padx=2)

        # Метка для отображения имени текущего режима
        self.current_mode_label = ttk.Label(mode_control_frame, text="No mode", 
                                            font=("Arial", 9, "bold"), 
                                            foreground="blue", width=25, anchor="center")
        self.current_mode_label.pack(side=tk.LEFT, padx=5)

        # Кнопка "→"
        self.btn_next_mode = ttk.Button(mode_control_frame, text="▶", width=3,
                                        command=self._next_mode, state=tk.DISABLED)
        self.btn_next_mode.pack(side=tk.LEFT, padx=2)

        # Кнопка "Apply Mode"
        self.btn_apply_mode = ttk.Button(mode_control_frame, text="Apply Mode",
                                        command=self._apply_current_mode)
        self.btn_apply_mode.pack(side=tk.LEFT, padx=10)
        ...
        column2 = ttk.Frame(main_frame, width=600)
        ...
        # === КОЛОНКА 3: Заданные выходы (Setpoints) ===
        column3 = ttk.Frame(main_frame, width=300)
        column3.pack(side=tk.LEFT, fill=tk.BOTH, expand=False, padx=(0, 10))
        column3.pack_propagate(False)
        
        setpoint_frame = ttk.LabelFrame(column3, text="Заданные выходы (Setpoints)", padding="5")
        setpoint_frame.pack(fill=tk.BOTH, expand=True)
        
        setpoint_container = ttk.Frame(setpoint_frame)
        setpoint_container.pack(fill=tk.BOTH, expand=True)
        
        # Создаем индикаторы для заданных выходов
        self.setpoint_labels = []
        self.setpoint_canvases = []
        
        for idx in range(16):
            cell_frame = ttk.Frame(setpoint_container, height=35)
            ...
            lbl = ttk.Label(cell_frame, text=f"SP{idx+1}: OFF", font=("Arial", 10))
            lbl.pack(side=tk.LEFT, padx=5)
            
            self.setpoint_canvases.append((canvas, circle_id))
            self.setpoint_labels.append(lbl)
        
        # === КОЛОНКА 4: Ожидаемые выходы (Expected) ===
        column4 = ttk.Frame(main_frame, width=300)
        column4.pack(side=tk.LEFT, fill=tk.BOTH, expand=False, padx=(0, 10))
        column4.pack_propagate(False)
        
        expected_frame = ttk.LabelFrame(column4, text="Ожидаемые выходы (Expected)", padding="5")
        expected_frame.pack(fill=tk.BOTH, expand=True)
        
        expected_container = ttk.Frame(expected_frame)
        expected_container.pack(fill=tk.BOTH, expand=True)
        
        # Создаем индикаторы для ожидаемых выходов
        self.expected_labels = []
        self.expected_canvases = []
        
        for idx in range(16):
            cell_frame = ttk.Frame(expected_container, height=35)
            cell_frame.pack(fill=tk.X, pady=3)
            cell_frame.pack_propagate(False)
            ...
            lbl = ttk.Label(cell_frame, text=f"EX{idx+1}: OFF", font=("Arial", 10))
            lbl.pack(side=tk.LEFT, padx=5)
            
            self.expected_canvases.append((canvas, circle_id))
            self.expected_labels.append(lbl)
        
        # === КОЛОНКА 5: Бинарные входы (Фактические) - КРАЙНЯЯ СПРАВА ===
        column5 = ttk.Frame(main_frame, width=300)
        column5.pack(side=tk.LEFT, fill=tk.BOTH, expand=False)
        column5.pack_propagate(False)
        
        inputs_frame = ttk.LabelFrame(column5, text="Бинарные входы (Фактические)", padding="5")
    ...
    def _load_mode(self):
        ...
        filepath = filedialog.askopenfilename(
            title="Выберите файл режима (.xlsx)",
            filetypes=[
                ("Excel files", "*.xlsx *.xls"),
                ("All files", "*.*")
            ],
            initialdir=os.path.join(os.getcwd(), "modes") if os.path.exists(os.path.join(os.getcwd(), "modes")) else os.getcwd()
        )
    ...
    def _load_modes(self):
        """Загрузка нескольких режимов из выбранной папки"""
        ...
        try:
            ...
            folder_path = filedialog.askdirectory(
                title="Выберите папку с файлами режимов",
                initialdir=os.path.join(os.getcwd(), "modes") if os.path.exists(os.path.join(os.getcwd(), "modes")) else os.getcwd(),
                mustexist=True
            )
            ...
            # Проверяем, что режимы загружены
            if self.modes and self.modes.get_modes_count() > 0:
                # Создаем панель управления режимами (если еще не создана)
                self._create_mode_control_panel()
                
                # Загружаем первый режим
                self._load_current_mode()
            else:
                self.status_var.set("Status: No modes loaded")
                logger.warning("GUI", "No modes were loaded from the selected folder")
    ...
    def _update_mode_controls(self):
        """Обновляет состояние кнопок навигации по режимам"""
        # Проверяем, созданы ли кнопки
        if not hasattr(self, 'btn_prev_mode') or not hasattr(self, 'btn_next_mode'):
            return
        
        if not hasattr(self, 'modes') or not self.modes:
            return
        
        modes_count = self.modes.get_modes_count()
        if modes_count == 0:
            self.btn_prev_mode.config(state=tk.DISABLED)
            self.btn_next_mode.config(state=tk.DISABLED)
            if hasattr(self, 'current_mode_label'):
                self.current_mode_label.config(text="No modes")
            return
        
        # Обновляем состояние кнопок
        self.btn_prev_mode.config(state=tk.NORMAL if self.current_mode_index > 0 else tk.DISABLED)
        self.btn_next_mode.config(state=tk.NORMAL if self.current_mode_index < modes_count - 1 else tk.DISABLED)
        
        # Обновляем отображение имени режима
        if hasattr(self, 'current_mode_label'):
            mode_names = self.modes.get_mode_names()
            if mode_names and self.current_mode_index < len(mode_names):
                current_name = mode_names[self.current_mode_index]
                self.current_mode_label.config(text=f"{current_name}")
                
                # Обновляем статусную строку
                self.status_var.set(f"Status: Active mode: {current_name}")

    def _prev_mode(self):
        """Переключение на предыдущий режим"""
        # Проверяем, созданы ли кнопки
        if not hasattr(self, 'btn_prev_mode'):
            return
    ...
    def _next_mode(self):
        ...
        # Проверяем, созданы ли кнопки
        if not hasattr(self, 'btn_next_mode'):
            return
    ...
    def _apply_current_mode(self):
        ...
        # Проверяем, созданы ли кнопки
        if not hasattr(self, 'btn_apply_mode'):
            return
        
        if not hasattr(self, 'modes') or not self.modes:
            messagebox.showwarning("Предупреждение", "Нет загруженных режимов")
            return
        
        if not hasattr(self, 'current_mode_index'):
            messagebox.showerror("Ошибка", "Индекс текущего режима не определен")
            return
        
        mode = self.modes.get_mode(self.current_mode_index)
        if not mode:
            messagebox.showerror("Ошибка", "Не удалось получить текущий режим")
            return
    ...
    def _run_autotest(self):
        """Запуск автотестирования (заглушка)"""
        logger.info("GUI", "Run Autotest button clicked (stub)")
        
        # Спрашиваем подтверждение
        if messagebox.askyesno("Подтверждение", "Запустить автоматическое тестирование?\n(Функция в разработке)"):
            self.status_var.set("Status: Running autotest... (stub)")
            logger.info("GUI", "Autotest started (stub)")
            
            # Имитация выполнения автотеста
            def simulate_autotest():
                time.sleep(0.5)
                logger.info("GUI", "Autotest step 1: Checking device connection (stub)")
                time.sleep(0.5)
                logger.info("GUI", "Autotest step 2: Verifying outputs (stub)")
                time.sleep(0.5)
                logger.info("GUI", "Autotest step 3: Checking binary inputs (stub)")
                time.sleep(0.5)
                logger.info("GUI", "Autotest completed successfully (stub)")
                self.status_var.set("Status: Autotest completed (stub)")
                messagebox.showinfo("Успех", "Автотестирование завершено (заглушка)")
            
            # Запускаем в отдельном потоке, чтобы не блокировать GUI
            threading.Thread(target=simulate_autotest, daemon=True).start()
        else:
            logger.info("GUI", "Autotest cancelled by user")
            self.status_var.set("Status: Autotest cancelled")
    ...
    def _execute_command(self, command: str):
        ...
        try:
            ...
            # Обработка команды ReadInputs
            if command == "ReadInputs":
                self.read_inputs()
                return
    ...
    def read_inputs(self):
        """Чтение состояния входных контактов через COM-объект"""
        try:
            # Проверяем, что устройство открыто
            if not self.retom_driver.is_open:
                logger.error("GUI", "Device not open, cannot read inputs")
                self.status_var.set("Status: Device not open")
                messagebox.showwarning("Предупреждение", "Устройство не подключено. Сначала нажмите Open.")
                return False
            
            # Вызываем метод драйвера
            contacts_mask = self.retom_driver.read_input_contacts(mode="mask")
            
            if contacts_mask is not None:
                # Обновляем GUI с полученной маской
                self._update_inputs_gui(contacts_mask)
                logger.info("GUI", f"Read inputs success: mask=0x{contacts_mask:04X}")
                self.status_var.set(f"Status: Inputs read (0x{contacts_mask:04X})")
                return True
            else:
                error_msg = self.retom_driver.st_error or "Unknown error"
                logger.error("GUI", f"Read inputs failed: {error_msg}")
                self.status_var.set(f"Status: Read inputs failed - {error_msg}")
                return False
                
        except Exception as e:
            logger.error("GUI", f"ReadInputs exception: {e}")
            self.status_var.set(f"Status: Read inputs error - {str(e)[:50]}")
            return False

    # ============ МЕТОДЫ ДЛЯ УПРАВЛЕНИЯ ЗАДАННЫМИ И ОЖИДАЕМЫМИ ВЫХОДАМИ ============
    
    def set_setpoint_output(self, output_number: int, state: bool):
        """
        Установка состояния заданного выхода
        
        Args:
            output_number: номер выхода (1-16)
            state: состояние (True=ON, False=OFF)
        """
        if 1 <= output_number <= 16:
            self.setpoint_outputs_state[output_number - 1] = state
            self._update_setpoint_outputs_gui()
            logger.info("GUI", f"Setpoint output {output_number} set to {'ON' if state else 'OFF'}")
        else:
            logger.error("GUI", f"Invalid output number: {output_number}")
    
    def set_setpoint_outputs_mask(self, mask: int):
        """
        Установка состояния заданных выходов по маске
        
        Args:
            mask: 16-битная маска состояния выходов
        """
        for i in range(16):
            self.setpoint_outputs_state[i] = bool(mask & (1 << i))
        self._update_setpoint_outputs_gui()
        logger.info("GUI", f"Setpoint outputs mask set to 0x{mask:04X}")
    
    def get_setpoint_outputs(self) -> List[bool]:
        """Получение текущего состояния заданных выходов"""
        return self.setpoint_outputs_state.copy()
    
    def get_setpoint_outputs_mask(self) -> int:
        """Получение маски заданных выходов"""
        mask = 0
        for i, state in enumerate(self.setpoint_outputs_state):
            if state:
                mask |= (1 << i)
        return mask
    
    def set_expected_output(self, output_number: int, state: bool):
        """
        Установка состояния ожидаемого выхода
        ...
        """
        if 1 <= output_number <= 16:
            self.expected_outputs_state[output_number - 1] = state
            self._update_expected_outputs_gui()
            logger.info("GUI", f"Expected output {output_number} set to {'ON' if state else 'OFF'}")
        else:
            logger.error("GUI", f"Invalid output number: {output_number}")
    
    def set_expected_outputs_mask(self, mask: int):
        """
        Установка состояния ожидаемых выходов по маске
        
        Args:
            mask: 16-битная маска состояния выходов
        """
        for i in range(16):
            self.expected_outputs_state[i] = bool(mask & (1 << i))
        self._update_expected_outputs_gui()
        logger.info("GUI", f"Expected outputs mask set to 0x{mask:04X}")
    
    def get_expected_outputs(self) -> List[bool]:
        """Получение текущего состояния ожидаемых выходов"""
        return self.expected_outputs_state.copy()
    
    def get_expected_outputs_mask(self) -> int:
        """Получение маски ожидаемых выходов"""
        mask = 0
        for i, state in enumerate(self.expected_outputs_state):
            if state:
                mask |= (1 << i)
        return mask
    ...
    def compare_outputs(self) -> dict:
        """
        Сравнение заданных и ожидаемых выходов с фактическими входами
        ...
        result = {
            'setpoint_vs_input': [],
            'expected_vs_input': [],
            'all_match_setpoint': True,
            'all_match_expected': True
        }
        
        for i in range(16):
            # Сравнение заданного с фактическим
            sp_match = (self.setpoint_outputs_state[i] == self.inputs_state[i])
            result['setpoint_vs_input'].append(sp_match)
            if not sp_match:
                result['all_match_setpoint'] = False
            
            # Сравнение ожидаемого с фактическим
            ex_match = (self.expected_outputs_state[i] == self.inputs_state[i])
            result['expected_vs_input'].append(ex_match)
            if not ex_match:
                result['all_match_expected'] = False
        
        return result
    
    def check_outputs_and_log(self):
        """Проверяет соответствие выходов и логирует результат"""
        comparison = self.compare_outputs()
        
        if comparison['all_match_setpoint']:
            logger.info("GUI", "✓ All setpoint outputs match actual inputs")
        else:
            mismatches = [i+1 for i, match in enumerate(comparison['setpoint_vs_input']) if not match]
            logger.warning("GUI", f"✗ Setpoint mismatches on outputs: {mismatches}")
        
        if comparison['all_match_expected']:
            logger.info("GUI", "✓ All expected outputs match actual inputs")
        else:
            mismatches = [i+1 for i, match in enumerate(comparison['expected_vs_input']) if not match]
            logger.warning("GUI", f"✗ Expected mismatches on outputs: {mismatches}")
    ...
    def _update_setpoint_outputs_text(self, labels_list: List[str]):
        ...
        Args:
            labels_list: список строк с названиями для выходов (макс. 16)
                        Если значений меньше 16, остальные получают текст "Не назначено"
        """
        # Создаем полный список из 16 элементов
        full_labels = ["Не назначено"] * 16
        ...
        for i, label in enumerate(labels_list):
            if i < 16:
                ...
                logger.warning("GUI", f"Too many labels provided, truncating to 16")
                break
    ...
    def _update_expected_outputs_text(self, labels_list: List[str]):
        """
        Обновляет текстовые подписи для ожидаемых выходов
    ...
    def _update_inputs_text(self, labels_list: List[str]):
        """
        Обновляет текстовые подписи для бинарных входов
        
        Args:
            labels_list: список строк с названиями для входов (макс. 16)
                        Если значений меньше 16, остальные получают текст "Не назначено"
        """
        # Создаем полный список из 16 элементов
        full_labels = ["Не назначено"] * 16
    ...
    def _update_setpoint_outputs_gui(self):
        """Обновляет отображение заданных выходов в GUI"""
        for i in range(16):
            is_active = self.setpoint_outputs_state[i]
            
            canvas, circle_id = self.setpoint_canvases[i]
            lbl = self.setpoint_labels[i]
            
            # Используем желтый цвет для заданных выходов
            color = "#FF0000" if is_active else '#646464'
            text_status = "ON" if is_active else "OFF"
    ...
    def _update_expected_outputs_gui(self):
        ...
        for i in range(16):
            is_active = self.expected_outputs_state[i]
            
            canvas, circle_id = self.expected_canvases[i]
            lbl = self.expected_labels[i]
            
            # Используем голубой цвет для ожидаемых выходов
            color = "#FF0000" if is_active else '#646464'
            text_status = "ON" if is_active else "OFF"
